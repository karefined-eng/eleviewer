...
- **Lazy-Scope Import Contamination:** When a method is lazy-loaded (e.g., `open_web_tab`), ALL names it references — including lightweight helpers like `compact_toolbar_stylesheet` from `theme.py` — must be imported locally inside that method. Top-level module imports are unreliable inside functions containing `global` statements and local `from X import Y` blocks. This is NOT caught by import smoke tests; only runtime execution of the lazy path triggers the `NameError`.
